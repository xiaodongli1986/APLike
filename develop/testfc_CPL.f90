...
implicit none
  ...
  integer :: iz, i, j, i1i2s(N1,N2), i1,i2, npoint
  ...
  real(rt) :: DAs(nz), Hs(nz), DAsfc(nzfc), Hsfc(nzfc), chisqs(nz-1), chisqs__uncored(nz-1), &
    ...
    ommin, ommax, wmin, wmax, omfid, wamin, wamax, tmpx
...
!  ommin = 0.20d0; ommax = 0.30d0; wmin = -1.20d0; wmax = -0.80d0;
  omfid = 0.3121_rt
!  ommin = omfid-0.08_rt; ommax = omfid+0.08_rt; wmin = -1.40d0; wmax = -0.60d0; wamin = -1.1d0; wamax = 1.1d0;
  ommin = omfid-0.2_rt; ommax = omfid+0.2_rt; wmin = -1.60d0; wmax = -0.40d0; wamin = -3.0d0; wamax = 3.0d0;  
  npoint =  10000
  ...
  fcstr = trim(adjustl(fcstr))//'_CPL_10000p'
...
!  write(103,'(A)') '# big covmat chisq, 1900 mocks, uncored; nbins minor index, mucut major index:  ## omegam; w; chisq; sep_chisqs with mubin/mucuts ='//trim(adjustl(tmpstr2))
  ...
  do i = 1,npoint
  call random_number(tmpx); nowpar%omegam = (tmpx-0.5)*(ommax-ommin) + (ommax+ommin)*0.5
  call random_number(tmpx); nowpar%w = (tmpx-0.5)*(wmax-wmin) + (wmax+wmin)*0.5
  call random_number(tmpx); nowpar%wa = (tmpx-0.5)*(wamax-wamin) + (wamax+wamin)*0.5
!  nowpar%omegam = 0.1+0.01_rt*(i-1); nowpar%w = -1.5_rt+(j-1)*0.02
  if(debug) then; nowpar%omegam = 0.26; nowpar%w = -1.0; nowpar%wa = 0.0; endif
...
!  write(103,'(<3+N1*N2>f14.3,1x)') nowpar%omegam, nowpar%w, sum(chisqs__uncored),  sepchisqs_uncored
  write(104,'(<4+N1*N2>(e14.7,1x))') nowpar%omegam, nowpar%w, nowpar%wa, sum(chisqs),  sepchisqs
  write(*,'(f4.1,A,3f8.4,<nz-1>f10.1)') float(i)/dble(npoint)*100, '%   nowpar%omegam, nowpar%w, nowpar%wa, Chisq value = ', &
    nowpar%omegam, nowpar%w, nowpar%wa, real(chisqs)
