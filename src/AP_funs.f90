...
!! Binning of xi (\mu) 
!!  Using 20-25 bins in mu space
!!  Larger mubin captures more information -> yields tighter cosmological constraints, 
!!   but the analysis becomes more difficult 
...
!!  Setting: directory for covmat files, chisq files
  ...
  character(len=charlen), parameter :: covmatdir = '/home/xiaodongli/software/APLike/covmat_files_save/', &
...
!! Path of 2-ponit correlation function (2PCF) files
  character(len=charlen), parameter :: filedir = '/home/xiaodongli/software/APLike/'
...
contains
 ...
 subroutine AP_like(DAs, Hs, chisqs,  printinfo)
    implicit none
    real(rt), intent(in) :: DAs(nz), Hs(nz)
    logical, intent(in) :: printinfo
    real(rt), intent(out):: chisqs(nz-1)
    real(rt) :: sepchisqs_uncored(n1,n2,nz-1), sepchisqs(n1,n2,nz-1), &
      chisqs_uncored(nz-1), fact
...
!    call system('sleep 0'); print *, 'Compute/output covmats...';call calc_covmats();call output_covmats()
      ...
      call calc_syscor()
      allocate(global_smutabstds(nbins_database,mubins_database,3,nz,num_fidcos))
    endif
    ! End 
    !--------------------------------
    ...
    fact = (4.0/5.0) ! Correction factor due to finite size of Horizon Run 4 simulation (used as correction of systematics)
    ...
    if(printinfo) then
      print *, 'Sys-corred:   ', real(chisqs(1:nz-1)),  sum(chisqs(1:nz-1))
      print *, 'Sys-uncorred: ', real(chisqs_uncored(1:nz-1)),  sum(chisqs_uncored(1:nz-1))
    endif
