...
!    enddo
  ...
  subroutine smu_ximu_CalcOmWChisqs(& 
    ...
    if(NBComp) then
      print *, 'ERROR! (smu_ximu_CalcOmWChisqs) NBComp not supported! Found NBComp = ', NBComp 
      stop
    endif
    if(Forecast) then
...
!     call system('sleep 0'); print *, 'Compute/output covmats...';call calc_covmats();call output_covmats()
      ...
      if(dataself_IO_test) then
       do i = 1, 10
         write(*,'(A)'), 'WARNING (AP_like)!!! Found dataself_IO_test set as .true.; pre-load global_smutabstds for self-syscor!!!'
       enddo
       do iz = 1, nz
